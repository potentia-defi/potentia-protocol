...
contract PotentiaPoolTest is Test {
    ...
    function test_Random() public {
        UD x = ud(1000000000000000000);
        UD R = ud(2475844975000000000);
        UD RMinusPhi = ud(50000000000000);
        uint256 p = 2e18;

        UD x_k = x.pow(ud(p));
        console.log("x power k %s", toUint(x_k));

        UD denominator = ud(4e18) * RMinusPhi * x_k;
        console.log("den %s", toUint(denominator));

        UD numerator = R.pow(ud(2e18));
        console.log("numerator %s", toUint(numerator));

        // 30649041701163752570000
        uint256 alpha = toUint((R.pow(ud(2e18)) / (ud(4e18) * (RMinusPhi) * x.pow(ud(p)))));

        console.log("alpha %s", alpha);
    }

    function test_Hooray() public {
        (UD x,) = pool.getX();
        ...
        console.log(
            "###### going into the future by 10000 seconds and changing the price from $2500 to $3000 ###### \n"
        );
        vm.warp(10000);
    ...
    function test_OpenLongPosition() public {
        ...
        console.log("Params after adding 50 ether liquidity at t=2");
        (x,) = pool.getX();
        ...
        // Take 10 ether Long Position
        underlying.approve(address(pool), 10 ether);
        pool.openPosition(10 ether, true);

        console.log("Params after taking 10 ether Long Position at t=2");
        (x,) = pool.getX();
        liq0 = toUint(pool.liquidity(x));
        phi = toUint(pool.longPayoff(x));
        psi = toUint(pool.shortPayoff(x));
        uint256 longValue = _positionValue(true, pool.longPToken().balanceOf(users[2]));
        uint256 longPrice = _positionValue(true, 1e18);
        console.log("LC = %s", toUint(pool.longCondition()));
        console.log("SC = %s", toUint(pool.shortCondition()));
        console.log("Liq = %s, Phi = %s, Psi = %s", liq0, phi, psi);
        console.log("Long Value = %s", longValue);
        console.log("LongSupply", pool.longPToken().totalSupply());
        console.log("Long Price = %s\n", longPrice);

        assertLe(longValue, 10 ether);
        ...
        longPrice = _positionValue(true, 1e18);
        console.log("LC = %s", toUint(pool.longCondition()));
        console.log("SC = %s", toUint(pool.shortCondition()));
        console.log("Liq = %s, Phi = %s, Psi = %s", liq0, phi, psi);
        console.log("Long Value = %s", longValue);
        ...
        // Take another 1000 ether Long Position
        underlying.approve(address(pool), 1000 ether);
        pool.openPosition(1000 ether, true);

        console.log("Params after taking 1000 ether Long Position at t=5000");
        (x,) = pool.getX();
        liq0 = toUint(pool.liquidity(x));
        phi = toUint(pool.longPayoff(x));
        psi = toUint(pool.shortPayoff(x));
        ...
        pool.withdraw();
        // assertEq(underlying.balanceOf(users[0]), protocolBal + 0.3 ether);
        vm.stopPrank();
    }
    ...
    function test_OpenShortPosition() public {
        ...
        assertLe(shortValue, 100 ether);
        assertEq(expectedFreeGrowth, toUint(pool.feeGrowth()));

        vm.warp(500);

        underlying.approve(address(pool), 200 ether);
        pool.openPosition(200 ether, false);
        ...
        uint256 protocolBal = underlying.balanceOf(users[0]);
        pool.withdraw();
        assertEq(underlying.balanceOf(users[0]), protocolBal + 3 ether);
        vm.stopPrank();
    }
